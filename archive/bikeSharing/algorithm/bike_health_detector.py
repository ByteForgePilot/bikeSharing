"""
共享单车健康快速检测系统 — 核心算法实现 (v4.0)

根据设计文档 v2 的三级检测架构：
  F1 — 轮胎偏摆检测：IMU Z轴 → FFT → 轮频 f 与 2f 幅值 → P=A1+0.5*A2
  F2 — 链条异响检测：8kHz 音频 → 包络谱分析 → 置信度评分
  F3 — 车头不正检测：GPS航向角 vs IMU偏航角偏差 Δθ → 线性插值评分
...
v4.0 更新：
  - 支持 GPS 数据输入 (速度 + 航向角)
  - F1: GPS 速度直接计算轮频先验，缩小 FFT 搜索范围
  - F3: GPS 航向角 vs IMU 偏航角直接对比，取代纯 IMU 估计
  - 窗口选取: GPS 速度辅助筛选匀速段
...
_PARENT_DIR = _os.path.dirname(_BASE_DIR)

SENSOR_FILE = _os.path.join(_PARENT_DIR, "传感器数据.txt")
PCM_FILE = _os.path.join(_PARENT_DIR, "音频.pcm")
AUDIO_TS_FILE = _os.path.join(_PARENT_DIR, "音频_时间戳.csv")


def load_sensor_data(path: str) -> tuple[list[AccelSample], list[GyroSample], list[GpsSample]]:
    """加载传感器数据。返回 (加速度计, 陀螺仪, GPS)。

    支持两种格式：
      v2 (无GPS): timestamp, 加速度计|陀螺仪, ax,ay,az, 空×4, gx,gy,gz
      v4 (有GPS): timestamp, 加速度计|陀螺仪|GPS, ax,ay,az, lat,lon,speed,course, gx,gy,gz
    自动检测: 若存在 GPS 行类型或速度字段非空则启用 GPS 解析。
    """
    ...
    for line in lines[1:]:
        ...
        if stype == "加速度计":
            ...
            gps.append(GpsSample(ts, lat, lon, speed, course))
...
def estimate_wheel_frequency(
    ...
    if speed_ms is not None and speed_ms > 0.5:
        # GPS 先验：取最接近期望频率的峰，而非最强峰
        # (最强峰可能是路面颠簸共振或倍频)
        f_expected = speed_ms / (2.0 * np.pi * WHEEL_RADIUS)
        lo = max(f_expected - 2.0, 1.0)
        hi = min(f_expected + 2.0, 10.0)
        mask = (freqs >= lo) & (freqs <= hi)
        if mask.any() and np.sum(mask) > 0:
            candidates_f = freqs[mask]
            idx = int(np.argmin(np.abs(candidates_f - f_expected)))
            return float(candidates_f[idx])
    else:
        lo, hi = 1.5, 8.0
        mask = (freqs >= lo) & (freqs <= hi)
        if mask.any():
...
def select_analysis_window(
    ...
    # GPS 匀速段评分 (有 GPS 时加权)
    if gps and len(gps) >= 3:
        gps_t = np.array([(g.timestamp_ns - t0) * 1e-9 for g in gps], dtype=np.float64)
        speed_arr = np.array([g.speed_ms for g in gps], dtype=np.float64)
        # 仅当 GPS 时间范围与加速度计有足够重叠时才使用
        if gps_t[-1] > t_uniform[0] and gps_t[0] < t_uniform[-1]:
            speed_uniform = np.interp(t_uniform, gps_t, speed_arr)
            if len(speed_uniform) >= window_samples:
                speed_windows = sliding_window_view(speed_uniform, window_samples)[::step_samples]
                speed_vars = speed_windows.var(axis=1)
                max_speed_var = max(float(np.max(speed_vars)), 0.01)
                speed_vars /= max_speed_var  # 归一化到 0~1
            else:
                speed_vars = np.zeros_like(az_vars)
        else:
            speed_vars = np.zeros_like(az_vars)
    else:
        speed_vars = np.zeros_like(az_vars)

    # 综合评分：路面越平 + 骑得越直 + 速度越稳 → 分越低 → 越好
    # speed_vars 权重 5.0 使其与 gz_vars*10 同量级，优选匀速骑行段
    scores_vec = az_vars * 1.0 + gz_vars * 10.0 + speed_vars * 5.0
...
def detect_tire_wobble(
    ...
    # 平整路面占比越低 → 置信度越低 → 平滑降权
    # flat=0 → weight=0.7, flat=1.0 → weight=1.0
    # (GPS 选频已大幅降低路面噪声干扰，故 flat 权重不需太激进)
    confidence = 0.7 + 0.3 * flat_fraction
    score *= confidence
...
def _check_harmonics(
    ...
    噪声基底从 0.3~15Hz 包络谱中取中位数 (排除基频和谐波位置)。
    """
    n = len(envelope)
    mag = np.abs(fft(envelope))[: n // 2]
    freqs = fftfreq(n, 1.0 / fs)[: n // 2]

    # 噪声基底：0.3~15Hz 范围，排除基频及其谐波
    noise_mask = (freqs >= 0.3) & (freqs <= 15.0)
    noise_mask &= ~((freqs >= fundamental - margin) & (freqs <= fundamental + margin))
    for h in range(2, n_harmonics + 2):  # 多排除一个谐波
        hf = fundamental * h
        noise_mask &= ~((freqs >= hf - margin) & (freqs <= hf + margin))
...
def detect_handlebar_misalignment(
    ...
    """
    车头不正检测 (F3) — v4.0 GPS 增强版

    原理：车头偏斜 → IMU 偏航角与 GPS 航向角存在系统性偏差 Δθ。

    GPS 模式 (有 GPS 数据时):
      ① 找到满足"直行条件"的 GPS 段: 速度≥2m/s 且 航向变化率<3°/s
      ② 在陀螺仪时间轴上积分偏航角
      ③ 对比 IMU 偏航角 vs GPS 航向角 → 平均偏差 Δθ
      ④ 这是设计文档中的原始算法，有 ground truth

    降级模式 (无 GPS 时):
      取 gz 方差最小的 30% 窗口作为"准直行段"，用 gz 偏置间接估计 Δθ。
    """
    if len(gyro) < 32:
        return {"score": 100.0, "delta_theta_deg": 0.0, "yaw_bias_rad_s": 0.0,
                "straight_segments": 0, "method": "insufficient_data"}

    # ===================================================================
    # GPS 模式：GPS 航向角 vs IMU 偏航角直接对比
    # ===================================================================
    if gps and len(gps) >= 3:
        return _detect_handlebar_with_gps(gyro, gps)

    # ===================================================================
    # 降级模式：无 GPS，IMU 相对直行段估计
    # ===================================================================
    return _detect_handlebar_imu_only(gyro)


def _detect_handlebar_with_gps(
    gyro: list[GyroSample], gps: list[GpsSample]
) -> dict:
    """GPS 增强模式：直接对比 GPS 航向角与 IMU 积分偏航角。

    设计文档算法 Step-by-step:
      1. 骑行状态判定：车速 ≥ 2 m/s 且 航向变化率 < 3°/s
      2. 取满足条件的连续段 (≥ 5 秒) → 记录 Yaw(IMU) 与 Course(GPS)
      3. 计算最小夹角差 → 平均偏差 Δθ
    """
    # ---- 提取 GPS 时间序列 ----
    t0 = gyro[0].timestamp_ns
    gps_times = np.array([(g.timestamp_ns - t0) * 1e-9 for g in gps], dtype=np.float64)
    speeds = np.array([g.speed_ms for g in gps], dtype=np.float64)
    courses_raw = np.array([g.course_deg for g in gps], dtype=np.float64)

    # ---- 提取陀螺仪时间序列 ----
    gyro_times = np.array([(g.timestamp_ns - t0) * 1e-9 for g in gyro], dtype=np.float64)
    gz = np.array([g.gz for g in gyro], dtype=np.float64)

    # ---- 1. 骑行状态判定 ----
    # GPS 速度 ≥ 2 m/s
    fast_mask = speeds >= 2.0
    if np.sum(fast_mask) < 3:
        return {"score": 100.0, "delta_theta_deg": 0.0, "yaw_bias_rad_s": 0.0,
                "straight_segments": 0, "method": "gps_insufficient_speed",
                "warning": "GPS 车速不足 (全程 < 2 m/s)，无法判定直行段"}

    # 平滑航向角: 3 点移动中值滤波 (降 GPS 1Hz 噪声, 保留真实转弯)
    from scipy.ndimage import median_filter
    courses = median_filter(courses_raw, size=3).astype(np.float64)

    # 航向变化率 < 5°/s (放宽阈值适配 1Hz GPS 噪声)
    course_diffs = np.abs(np.diff(courses))
    # 处理 0°/360° 环绕
    course_diffs = np.minimum(course_diffs, 360.0 - course_diffs)
    dt_gps = np.diff(gps_times)
    dt_gps = np.maximum(dt_gps, 0.1)  # 防止除零
    course_rate = course_diffs / dt_gps  # °/s

    # 直行条件：速度快且航向稳定
    straight_mask = np.zeros(len(gps), dtype=bool)
    straight_mask[:-1] = fast_mask[:-1] & (course_rate < 5.0)
    straight_mask[-1] = straight_mask[-2]  # 最后一个点继承前一个

    if np.sum(straight_mask) < 3:
        return {"score": 100.0, "delta_theta_deg": 0.0, "yaw_bias_rad_s": 0.0,
                "straight_segments": 0, "method": "gps_no_straight",
                "warning": "无满足条件的直行段 (航向变化率 > 3°/s)"}

    # ---- 2. 取最长的连续直行段 ----
    # 将 straight_mask 按连续 True 分组
    from itertools import groupby

    straight_segments_gps = []
    for k, group in groupby(enumerate(straight_mask), key=lambda x: x[1]):
        if k:  # True
            indices = [i for i, _ in group]
            straight_segments_gps.append((len(indices), indices[0], indices[-1]))

    if not straight_segments_gps:
        return _detect_handlebar_imu_only(gyro)

    # 取最长的直行段
    straight_segments_gps.sort(key=lambda x: x[0], reverse=True)
    _, seg_start, seg_end = straight_segments_gps[0]

    seg_t0 = gps_times[seg_start]
    seg_t1 = gps_times[seg_end]
    seg_duration = seg_t1 - seg_t0

    if seg_duration < 2.0:  # 至少需要 2 秒 (GPS 1Hz, 最少 2 个连续点)
        return {"score": 100.0, "delta_theta_deg": 0.0, "yaw_bias_rad_s": 0.0,
                "straight_segments": 0, "method": "gps_segment_too_short",
                "warning": f"最长直行段仅 {seg_duration:.1f}s (< 2s)"}

    # ---- 3. 积分 IMU 偏航角 (在直行段的时间范围内) ----
    gyro_in_seg = (gyro_times >= seg_t0) & (gyro_times <= seg_t1)
    if np.sum(gyro_in_seg) < 16:
        return _detect_handlebar_imu_only(gyro)

    # 积分 gz 得到 IMU 偏航角变化
    gz_seg = gz[gyro_in_seg]
    gyro_t_seg = gyro_times[gyro_in_seg]
    dt_gyro = np.diff(gyro_t_seg)
    yaw_delta = np.cumsum(gz_seg[:-1] * dt_gyro)  # rad
    yaw_delta_deg = np.degrees(yaw_delta)          # °
    yaw_delta_t = gyro_t_seg[1:]

    # ---- 4. 对比 GPS 航向与 IMU 偏航 ----
    # 插值 GPS 航向角到 IMU 时间轴上
    gps_course_seg = courses[seg_start:seg_end+1]
    gps_t_seg = gps_times[seg_start:seg_end+1]

    if len(gps_t_seg) >= 2:
        gps_course_interp = np.interp(yaw_delta_t, gps_t_seg, gps_course_seg)
    else:
        gps_course_interp = np.full_like(yaw_delta_t, gps_course_seg[0])

    # 对齐初始航向：设直行段起点处 IMU yaw=0, GPS course=course₀
    # IMU 偏航角变化 vs GPS 航向变化 应该是 1:1 的
    # Δθ = mean(GPS_course_change - IMU_yaw_change)
    course_change = gps_course_interp - gps_course_interp[0]
    # 处理环绕
    course_change = np.where(course_change > 180, course_change - 360, course_change)
    course_change = np.where(course_change < -180, course_change + 360, course_change)

    # 偏差序列
    deviations = np.abs(course_change - yaw_delta_deg)

    # 取中位偏差（比均值更抗噪声）
    delta_theta_deg = float(np.median(deviations))
    ...
    return {
        ...
        "yaw_bias_rad_s": 0.0,  # GPS 模式下不使用此指标
        "straight_segments": len(straight_segments_gps),
        "method": "gps_course_vs_imu",
        "gps_segment_duration_s": round(seg_duration, 1),
        "num_gps_straight_segments": len(straight_segments_gps),
    }


def _detect_handlebar_imu_only(gyro: list[GyroSample]) -> dict:
    """IMU 降级模式：无 GPS 时用 gz 偏置间接估计 Δθ。"""
    if len(gyro) < 32:
        return {"score": 100.0, "delta_theta_deg": 0.0, "yaw_bias_rad_s": 0.0,
                "straight_segments": 0, "method": "imu_fallback"}
    ...
    if not segments:
        return {"score": 100.0, "delta_theta_deg": 0.0, "yaw_bias_rad_s": 0.0,
                "straight_segments": 0, "method": "imu_fallback_no_segments"}
    ...
    if best_std > MAX_STRAIGHT_STD:
        return {
            "score": 100.0, "delta_theta_deg": 0.0, "yaw_bias_rad_s": 0.0,
            "straight_segments": 0, "method": "imu_fallback_no_straight",
    ...
    yaw_bias = float(np.mean(gz_means))
    avg_yaw_deviation = float(np.mean([abs(d) for d in yaw_deviations]))
...
def main() -> dict | None:
    ...
    print("=" * 60)
    print("  共享单车健康快速检测系统  v4.0")
    print("  Bike Health Rapid Inspection System")
    print("=" * 60)

    # 加载
    print("\n[1/4] 加载传感器数据 ...")
    accel, gyro, gps = load_sensor_data(SENSOR_FILE)
    print(f"      加速度计: {len(accel)} 条 | 陀螺仪: {len(gyro)} 条", end="")
    if gps:
        speeds = [g.speed_ms for g in gps if g.speed_ms > 0]
        avg_spd = sum(speeds) / len(speeds) if speeds else 0.0
        print(f" | GPS: {len(gps)} 条 | 平均车速: {avg_spd:.1f} m/s")
    else:
        print()
    ...
    if gps and window_result.get("used_window"):
        ...
        gps_slice = [g for g in gps if ws_ns <= g.timestamp_ns < we_ns]

    # ---- 计算 GPS 平均车速 (F1 先验) ----
    gps_speeds = [g.speed_ms for g in gps_slice if g.speed_ms > 0.5] if gps_slice else []
    ...
    print("  ├─ F1 轮胎偏摆检测 ...")
    f1 = detect_tire_wobble(accel, gyro, speed_ms=avg_speed_ms)
    if avg_speed_ms:
        print(f"  │   GPS 平均车速: {avg_speed_ms:.1f} m/s "
              f"→ 期望轮频: {avg_speed_ms/(2*np.pi*WHEEL_RADIUS):.1f} Hz")
    ...
    print("  └─ F3 车头不正检测 ...")
    f3 = detect_handlebar_misalignment(gyro, gps_slice if gps_slice else None)
    method_label = f3.get("method", "unknown")
    if method_label == "gps_course_vs_imu":
        print(f"      [GPS模式] GPS航向 vs IMU偏航 | 直行段: {f3.get('num_gps_straight_segments', 0)}")
        print(f"      GPS直行段时长: {f3.get('gps_segment_duration_s', 0):.1f}s")
    elif method_label.startswith("gps_"):
        print(f"      [GPS模式] {f3.get('warning', '')}")
    else:
        print(f"      [IMU降级模式]")
