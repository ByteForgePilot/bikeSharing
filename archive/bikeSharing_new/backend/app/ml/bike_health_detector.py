...
def detect_chain_noise(
    ...
    # 12. 连续评分 + 三级分级（分段映射）
    #
    #   异常分 < 0.30 → 100（正常，无周期结构）
    #   0.30 ~ 0.50 → 100~70（轻微周期成分 → "良好"下限）
    #   0.50 ~ 0.70 → 70~30（中等周期性 → "轻微"区间）
    ...
    if anomaly_score < 0.25:
        ...
        score = 100.0 - 25.0 * (anomaly_score - 0.25) / 0.25
    elif anomaly_score < 0.70:
        score = 70.0 - 40.0 * (anomaly_score - 0.50) / 0.20
    else:
...
def compute_health_score(
    ...
    penalty = (min(s1, s2, s3) / 100.0) ** 0.7

    # 最终评分：调和平均 × 温和最小值惩罚（^0.7 比 ^0.6 更轻）
    S = H * penalty
